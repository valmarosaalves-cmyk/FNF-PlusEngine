...
import funkin.ui.debug.charting.components.MetaNote.EventMetaNote;
import funkin.ui.debug.charting.components.VSlice.VSlicePackage;
import funkin.ui.debug.charting.components.VSlice.VSliceMetadata;
...
class ChartEditorState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	function hasLoadedChartAudio():Bool
	{
		return PlayState.SONG != null && FlxG.sound.music != null && FlxG.sound.music.length > 0;
	}

	function fitFileTabButtonWidth(button:PsychUIButton, baseWidth:Int):Void
	{
		if (button == null || button.text == null || button.bg == null)
			return;

		var buttonHeight:Int = Std.int(Math.max(20, button.bg.height));
		var maxWidth:Int = Std.int(Math.max(baseWidth, upperBox.width - button.x - 6));
		var targetWidth:Int = baseWidth;

		while (targetWidth < maxWidth && button.text.height > button.bg.height - 4)
		{
			targetWidth = Std.int(Math.min(targetWidth + 20, maxWidth));
			button.resize(targetWidth, buttonHeight);
			button.text.alignment = LEFT;
		}
	}

	function refreshFileTabButtonWidths(tabGroup:Dynamic, baseWidth:Int):Void
	{
		if (tabGroup == null || tabGroup.members == null)
			return;
		...
		for (member in members)
		{
			...
			fitFileTabButtonWidth(cast member, baseWidth);
		}
	}

	function countSongNotes(song:SwagSong):Int
	{
	...
	function promptStepManiaImportOptions(importedSong:SwagSong, fileName:String, difficultyName:String):Void
	{
		var noteCount:Int = countSongNotes(importedSong);
		if (noteCount < 1)
		{
			showOutput('Error: No notes were found in the selected StepMania difficulty.', true);
			return;
		}

		openSubState(new BasePrompt(460, 220, 'Import StepMania Notes', function(state:BasePrompt)
		{
			var description:FlxText = new FlxText(state.bg.x + 20, state.bg.y + 55, state.bg.width - 40,
				'Found $noteCount notes in "$difficultyName".\nOnly note data will be imported; audio, events, and chart metadata stay unchanged.', 16);
			description.alignment = CENTER;
			description.cameras = state.cameras;
			state.add(description);

			var btnY:Float = state.bg.y + state.bg.height - 55;
			var replaceBtn:PsychUIButton = new PsychUIButton(state.bg.x + 20, btnY, 'Replace Notes', function()
			{
				state.close();
				importStepManiaNotes(importedSong, true, fileName, difficultyName);
			}, 130);
			replaceBtn.normalStyle.bgColor = FlxColor.RED;
			replaceBtn.normalStyle.textColor = FlxColor.WHITE;
			replaceBtn.cameras = state.cameras;
			state.add(replaceBtn);

			var addBtn:PsychUIButton = new PsychUIButton(state.bg.x + 165, btnY, 'Add Notes', function()
			{
				state.close();
				importStepManiaNotes(importedSong, false, fileName, difficultyName);
			}, 120);
			addBtn.normalStyle.bgColor = FlxColor.GREEN;
			addBtn.normalStyle.textColor = FlxColor.WHITE;
			addBtn.cameras = state.cameras;
			state.add(addBtn);

			var cancelBtn:PsychUIButton = new PsychUIButton(state.bg.x + 300, btnY, 'Cancel', state.close, 120);
			cancelBtn.cameras = state.cameras;
			state.add(cancelBtn);
		}));
	}

	function promptStepManiaDifficulty(smFile:SMFile, fileName:String):Void
	{
		if (smFile == null || !smFile.isValid || smFile.difficulties == null || smFile.difficulties.length < 1)
		{
			showOutput('Error: File loaded is not a valid StepMania SM chart.', true);
			return;
		}

		if (smFile.difficulties.length == 1)
		{
			var importedSong:SwagSong = smFile.convertToFNF(PlayState.SONG.song, 0);
			if (importedSong == null)
			{
				showOutput('Error: Failed to convert the selected StepMania difficulty.', true);
				return;
			}

			promptStepManiaImportOptions(importedSong, fileName, smFile.difficulties[0].name);
			return;
		}

		var promptHeight:Int = 150 + (smFile.difficulties.length * 28);
		openSubState(new BasePrompt(460, promptHeight, 'Choose a StepMania Difficulty', function(state:BasePrompt)
		{
			var startY:Float = state.bg.y + 55;
			for (i => difficulty in smFile.difficulties)
			{
				var diffIndex:Int = i;
				var diffName:String = difficulty.name;
				var diffButton:PsychUIButton = new PsychUIButton(state.bg.x + 20, startY + (i * 28), '  ' + diffName, function()
				{
					var importedSong:SwagSong = smFile.convertToFNF(PlayState.SONG.song, diffIndex);
					if (importedSong == null)
					{
						showOutput('Error: Failed to convert the selected StepMania difficulty.', true);
						return;
					...
					state.close();
					promptStepManiaImportOptions(importedSong, fileName, diffName);
				}, Std.int(state.bg.width - 40));
				diffButton.text.alignment = LEFT;
				diffButton.cameras = state.cameras;
				state.add(diffButton);
			}

			var cancelBtn:PsychUIButton = new PsychUIButton(state.bg.x + state.bg.width - 110, state.bg.y + state.bg.height - 35, 'Cancel', state.close, 90);
			cancelBtn.cameras = state.cameras;
			state.add(cancelBtn);
		}));
	}

	function importStepManiaNotes(importedSong:SwagSong, replaceExisting:Bool, fileName:String, difficultyName:String):Void
	{
		if (PlayState.SONG == null || PlayState.SONG.notes == null)
		{
			showOutput('Error: Load a chart before importing StepMania notes.', true);
			return;
		}

		if (!hasLoadedChartAudio())
		{
			showOutput('Load at least one song before importing an SM file so the converted notes line up with the current chart timing.', true);
			return;
		}
		...
		{
			showOutput('Error: Current chart timing is unavailable.', true);
			return;
		}

		var importedNotes:Array<Array<Dynamic>> = [];
		for (section in importedSong.notes)
		{
			if (section == null || section.sectionNotes == null)
				continue;
			...
			for (note in section.sectionNotes)
			{
				...
				var noteData:Array<Dynamic> = cast(note, Array<Dynamic>).copy();
		...
		importedNotes.sort(PlayState.sortByTime);
		...
		var chartEndTime:Float = cachedSectionTimes[cachedSectionTimes.length - 1];
		...
		for (noteData in importedNotes)
		{
			...
			while (sectionIndex + 1 < cachedSectionTimes.length && cachedSectionTimes[sectionIndex + 1] <= noteTime)
				sectionIndex++;

			if (noteTime >= chartEndTime || sectionIndex >= PlayState.SONG.notes.length)
			{
				skippedCount++;
				continue;
			}

			PlayState.SONG.notes[sectionIndex].sectionNotes.push(noteData);
			importedCount++;
		}

		for (section in PlayState.SONG.notes)
			if (section != null && section.sectionNotes != null)
				section.sectionNotes.sort(PlayState.sortByTime);

		reloadNotes();
		forceDataUpdate = true;

		var actionText:String = replaceExisting ? 'replaced' : 'added';
		var message:String = 'SM notes ' + actionText + ' from ' + fileName + ' (' + difficultyName + '): ' + importedCount + ' imported';
		if (skippedCount > 0)
			message += ', ' + skippedCount + ' skipped outside the current chart length';
	...
	function addFileTab()
	{
		...
		btnY += 20;
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Load SM File...', function()
		{
			if (!fileDialog.completed)
				return;
			upperBox.isMinimized = true;
			...
			fileDialog.open('notes.sm', 'Open a StepMania SM file', [new flash.net.FileFilter('StepMania Chart', '*.sm')], function()
			{
				try
				{
					var filePath:String = fileDialog.path.replace('\\', '/');
					var fileName:String = filePath.substring(filePath.lastIndexOf('/') + 1);
					var smFile:SMFile = new SMFile(fileDialog.data, fileDialog.path);
					promptStepManiaDifficulty(smFile, fileName);
				}
				catch (e:Exception)
				{
					showOutput('Error: ${e.message}', true);
				}
			}, null, function() showOutput('Error: Could not open the StepMania file.', true));
		}, btnWid);
		btn.text.alignment = LEFT;
		tab_group.add(btn);
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Codename to Psych...', function()
		{
			...
			fileDialog.open('normal.json', 'Open a Codename Engine Chart file', function()
			{
				...
				try
				{
					chartData = Json.parse(fileDialog.data);
				}
				catch (e:Exception)
				{
				}
				...
				if (diffName == null || diffName.length < 1)
					diffName = 'normal';
				...
				var doConversion = function(?extraMeta:CodenameMetaData)
				{
					try
					{
						...
						fileDialog.openDirectory('Save Converted Psych JSON', function()
						{
							...
							overwriteSavedSomething = false;
							overwriteCheck(path + outFileName, outFileName, PsychJsonPrinter.print(chartSong, ['sectionNotes', 'events']), function()
							{
								if (pack.events != null)
								{
				...
				if (needsMeta)
				{
					fileDialog.open('meta.json', 'Open the Codename Engine meta.json file', function()
					{
						...
						try
						{
							meta = cast Json.parse(fileDialog.data);
						}
						catch (e:Exception)
						{
						}
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  NightmareVision to Psych...', function()
		{
			...
			fileDialog.open('songname.json', 'Open a Nightmare Vision Chart file', function()
			{
				...
				fileDialog.openDirectory('Save Converted Psych JSON', function()
				{
					...
					overwriteSavedSomething = false;
					overwriteCheck(path + outFileName, outFileName, PsychJsonPrinter.print(loadedChart, ['sectionNotes', 'events']), function()
					{
						if (overwriteSavedSomething)
							showOutput('File saved successfully to: ${fileDialog.path}!');
					}, true);
			...
			fileDialog.open('song.json', 'Open a psych_v1 chart file to convert', function()
			{
				try
				{
					...
					fileDialog.save(chartName, PsychJsonPrinter.print(v2Chart, ['notes', 'events', 'bpmChanges', 'characters']),
						function() showOutput('Chart converted and saved to: ${fileDialog.path}'), null,
		...
		btn.text.alignment = LEFT;
		tab_group.add(btn);
		refreshFileTabButtonWidths(tab_group, btnWid);
	}
	...
	function updateNotesRGB()
	{
		...
		for (note in notes)
		{
			...
			{
				var data = funkin.data.notestyle.NoteTypesConfig.loadNoteTypeData(note.noteType);
