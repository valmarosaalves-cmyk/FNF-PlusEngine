package funkin.play.stage.erect;
...
class LimoRideErect extends BaseStage
{
	...
	override function createPost()
	{
		...
		{
			grpLimoDancers.forEach(function(s:BackgroundDancer)
			{
				s.shader = colorShader;
			});
			...
			limo.shader = colorShader;
			bgLimo.shader = colorShader;
		...
		if(!ClientPrefs.data.lowQuality)
		{
			...
			switch(limoKillingState)
			{
				...
				case STOPPING:
					...
					if(Math.round(bgLimo.x) == -150) bgLimo.x = -150;
