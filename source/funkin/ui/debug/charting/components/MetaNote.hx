...
class MetaNote extends Note
{
	...
	public function setSustainLength(v:Float, stepCrochet:Float, zoom:Float = 1)
	{
		...
		if(sustainLength > 0)
		{
			...
			sustainSprite.setGraphicSize(8, Math.max(ChartEditorState.GRID_SIZE/4, (Math.round((v * ChartEditorState.GRID_SIZE + ChartEditorState.GRID_SIZE) / stepCrochet) * zoom) - ChartEditorState.GRID_SIZE/2));
